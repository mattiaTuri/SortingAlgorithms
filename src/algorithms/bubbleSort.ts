...
export const bubbleSort = async (index: number, speed: number) => {
  ...
  if (spanElem.length > 1) {
    for (let i: number = 0; i < spanElem.length; i++) {
      if (i !== spanElem.length - 1) {
        ...
        updateElemPos(currentNumber, nextNumber, "0", "-150", speed);
        await delay(speed);
        if (parseInt(currentNumber.id) > parseInt(nextNumber.id)) {
          const diff: number = nextNumber.offsetLeft - currentNumber.offsetLeft;
          updateElemPos(currentNumber, nextNumber, diff.toString(), "-150", speed);
          await delay(speed);
          updateElemPos(currentNumber, nextNumber, diff.toString(), "0", speed);
          await delay(speed);
          ...
          highlighElem(currentNumber, "#faa916");
          highlighElem(nextNumber, "#faa916");
          updateElemPos(currentNumber, nextNumber, "0", "0", speed);
          currentNumber = nextNumber;
        }
      }
      await delay(speed);
    }
    await bubbleSort(index + 1, speed);
  }
...
const setColorCompleteElem = (array: HTMLSpanElement[], index: number) => {
  array.splice(array.length - index, index).forEach((elem: HTMLSpanElement) => (elem.style.backgroundColor = "#20FC8F"));
};

const updateElemPos = (currentNumber: HTMLSpanElement, nextNumber: HTMLSpanElement, x: string, y: string, time: number) => {
...
const insertElementBefore = async (currentNumber: HTMLSpanElement, nextNumber: HTMLSpanElement) => {
